...
import { createTask, deleteTask, getProjectTasksPage, moveTaskToSprint, reorderTask, updateTask } from '../services/taskService';
...
export default function ProjectBacklogPage() {
  ...
  const tasksQuery = useInfiniteQuery({
    queryKey: queryKeys.projectTasksPage(projectId ?? '', 'backlog'),
    queryFn: ({ pageParam }) => getProjectTasksPage(projectId!, { cursor: pageParam, limit: 75 }),
    enabled: Boolean(projectId),
  ...
  useEffect(() => {
    ...
    function handleTaskCreated(task: Task) {
      if (task.projectId === projectId) {
        ...
        if (!task.parentId) queryClient.invalidateQueries({ queryKey: queryKeys.projectTasksPage(projectId, 'backlog') });
  ...
  async function handleDeleteSelected(taskIds: string[]) {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: queryKeys.projectTasksPage(projectId, 'backlog') });
  ...
  async function handleBulkUpdate(taskIds: string[], changes: Parameters<typeof updateTask>[1]) {
    ...
    try { const updated = await Promise.all(taskIds.map((id) => updateTask(id, changes))); queryClient.setQueryData<Task[]>(queryKeys.projectTasks(projectId!), (current = []) => current.map((task) => updated.find((item) => item.id === task.id) ?? task)); await queryClient.invalidateQueries({ queryKey: queryKeys.projectTasksPage(projectId!, 'backlog') }); toast.success(`${updated.length} tasks updated.`); }
  ...
  async function handleReorder(taskId: string, order: number) {
    try { const updated = await reorderTask(taskId, order); queryClient.setQueryData<Task[]>(queryKeys.projectTasks(projectId!), (current = []) => current.map((task) => task.id === updated.id ? updated : task)); await queryClient.invalidateQueries({ queryKey: queryKeys.projectTasksPage(projectId!, 'backlog') }); toast.success('Backlog priority updated.'); }
